...
export interface IAppStack {
  ...
  /**
   * Public domain for accessing the app (e.g. `app.example.com`)
   */
  domain: string;
  ...
  /**
   * Ingress and traffic exposure configuration
   */
  ingress?: IngressOptions;

  /**
   * CORS preset mode for frontend access.
   * This controls which frontend apps are allowed to access this service via browser.
   *
   * If defined, this will be resolved into Contour's `corsPolicy` on the HTTPProxy.
   */
  cors?: CorsPreset;
...
export interface IngressOptions {
  /**
   * Configuration for public (external) HTTPS access
   */
  external?: {
    /**
     * External HTTPS certificate issuer
     */
    certificate: {
      issuerName: string;
    };
  ...
  /**
   * Configuration for internal service communication (e.g. mTLS)
   */
  internal:
    | {
        /**
         * Protocol used for service-to-service calls
         */
        protocol: 'http';
      }
    | {
        ...
        /**
         * Internal TLS certificate for service validation
         */
        certificate: {
          /**
           * Secret containing CA certificate for validation
           */
          secretName: string;

          /**
           * TLS subjectName for internal mTLS verification
           *
           * e.g. `*.example.com` for wildcard domain
           */
          subjectName: string;
        };
      };
  ...
  /**
   * The backend port to route traffic to
   * This will be used in:
   * - Service targetPort
   * - Deployment containerPort
   * - HTTPProxy route port
   * @default 80
   */
  targetPort?: number;
  /**
   * Optional additional HTTPProxy route configuration
   */
  routeConfig?: NonNullable<IHTTPProxy['spec']['routes']>[number];
}
